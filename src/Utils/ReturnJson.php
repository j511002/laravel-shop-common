...
namespace SimpleShop\Commons\Utils;
...
class ReturnJson
{
     ...
     * @return JsonResponse
     */
    public static function success($data = '', $message = '操作已成功')
    {
        $reData = [
            'code'    => 0,
            'message' => $message,
     ...
     * @return JsonResponse
     */
    public static function fail($msg = 'Service Error', $status = 500)
    {
        if (is_string($msg)) {
            $messages = static::standardMessage($msg);
        } else {
            $messages = $msg;
        }
     ...
     * @return array
     */
    private static function standardMessage($msg)
    {
        if (is_string($msg)) {
            $msg = [
                'serverInternal' => [
